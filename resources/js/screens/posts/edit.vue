    ...
    <div>
        <page-header>
            ...
            <div class="flex items-center" v-if="ready && entry" slot="right-side">
                <button class="py-1 px-2 btn-primary text-sm mr-6" @click="publishingModal" v-if="!form.published">
                    Publish
                </button>
                <button class="py-1 px-2 btn-primary text-sm mr-6" @click="publishingModal" v-if="form.published">
                    Update
                ...
                <dropdown class="relative">
                    ...
                    <div slot="content" class="dropdown-content pin-r min-w-dropdown mt-1 text-sm py-2">
                        <a href="#" @click.prevent="settingsModal"
                           class="no-underline text-text-color hover:text-primary w-full block py-2 px-4">
                            General Settings
                        </a>
                        <a href="#" @click.prevent="featuredImageModal"
                           class="no-underline text-text-color hover:text-primary w-full block py-2 px-4">
                            Featured Image
                        </a>
                        <a href="#" @click.prevent="seoModal"
                           class="no-underline text-text-color hover:text-primary w-full block py-2 px-4">
                            SEO & Social
                        </a>
                        <a href="#" @click.prevent="deletePost" class="no-underline text-red w-full block py-2 px-4"
                           v-if="id != 'new'">Delete</a>
                    </div>
                </dropdown>
            </div>
        </page-header>
        ...
        <div class="container">
            ...
            <div class="lg:w-3/4 mx-auto" v-if="ready && entry">
                ...
                <div v-if="form.markdown == null">
                    <button class="w-full mb-5 hover:bg-lighter text-text-color block bg-very-light px-3 py-5 rounded"
                            @click="form.markdown = false">
                        I want a rich text editor
                    </button>
                    <button class="w-full mb-5 hover:bg-lighter text-text-color block bg-very-light px-3 py-5 rounded"
                            @click="form.markdown = true">
        ...
        <modal v-if="publishingModalShown" @close="publishingModalShown = false">
            ...
            <div class="mt-10">
                <button class="btn-sm btn-primary" @click="publishPost" v-if="!form.published" v-loading="status">
                    Publish this post
                </button>
                <button class="btn-sm btn-primary" @click="publishPost" v-if="form.published" v-loading="status">Update
                    Post
                </button>
                <button class="btn-sm ml-1 btn-light" @click="unpublishPost" v-if="form.published" v-loading="status">
                    Convert to draft
                </button>
